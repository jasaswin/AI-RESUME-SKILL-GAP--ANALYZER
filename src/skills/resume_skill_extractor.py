...
class ResumeSkillExtractor:
    ...
    def extract(self, tokens: list[str], resume_text: str = "") -> dict:
        ...
        # ✅ NEW: Auto-tokenization (CRITICAL FIX)
        words = set(re.findall(r"[a-zA-Z0-9\.]+", text_lower))

        # ---------- 1️⃣ Single-word + phrase detection ----------
        for skill in self.skill_db.skills:
            normalized_skill = self.skill_db.normalize_skill(skill)

            # match full word OR phrase
            if (
                skill in text_lower or
                normalized_skill in words
            ):
                skill_hits[normalized_skill] = {"source": "explicit"}

        # ---------- 2️⃣ Extra fallback for common skills ----------
        common_skills = ["python", "sql", "express", "nodejs", "react"]
        ...
        for dep in inferred_dependencies:
            normalized = self.skill_db.normalize_skill(dep)
            if normalized not in expanded_skills and self.skill_db.is_valid_skill(normalized):
                expanded_skills[normalized] = {"source": "inferred"}
